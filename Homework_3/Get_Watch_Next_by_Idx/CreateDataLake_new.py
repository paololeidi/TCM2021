...
tedx_dataset_agg = tedx_dataset.join(tags_dataset_agg, tedx_dataset.idx == tags_dataset_agg.idx_ref, "left") \
    .drop("idx_ref") \
    .select(col("idx").alias("_id"), col("url").alias("video_url"), col("*")) \
    .drop("idx") \
    .drop("url")
...
watch_next_dataset = spark.read.option("header","true").option("multiLine","true").csv(watch_next_dataset_path)
watch_next_dataset = watch_next_dataset.drop_duplicates().where('url != "https://www.ted.com/session/new?context=ted.www%2Fwatch-later"')

watch_next_dataset = watch_next_dataset.join(tedx_dataset_agg, watch_next_dataset.watch_next_idx == tedx_dataset_agg._id, "left") \
    .drop("_id") \
    .select(col("watch_next_idx"), col("*"))
...
watch_next_dataset_agg = watch_next_dataset.groupBy(col("idx").alias("idx_ref")) \
    .agg(collect_list(struct("watch_next_idx", "main_speaker", "title", "details", "posted", "url", "tags")).alias("watch_next_videos"))
